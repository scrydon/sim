...
vi.mock('@/lib/urls/utils', () => ({
  getEmailDomain: vi.fn().mockReturnValue('scrydon.eu'),
}))
  ...
  describe('sendEmail', () => {
    ...
    it('should send a marketing email with unsubscribe headers', async () => {
      ...
      expect(mockSend).toHaveBeenCalledWith({
        ...
        headers: {
          'List-Unsubscribe':
            '<https://test.scrydon.eu/unsubscribe?token=mock-token-123&email=test%40example.com>',
